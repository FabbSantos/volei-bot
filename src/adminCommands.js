...
async function processarComandoAdmin(msg) {
  ...
  if (matchSurpresaDe) {
    ...
    const aberta = db.getListaAtiva(r.grupo.chat_id);
    if (aberta) {
      return msg.reply(
    ...
    const { ja_existia, lista } = db.criarLista(r.grupo.chat_id, dataJogo, nomeLista, valorCriacao, { soFixos: true });
    if (ja_existia) {
    ...
    const anuncio =
      `⚡ *${nomeLista}* dia *${dataJogo}*${valor > 0 ? ` — ${db.formatarReais(valor)} por pessoa` : ''}!\n\n` +
    ...
    return msg.reply(
      ...
      `${valor > 0 ? ` — ${db.formatarReais(valor)}/pessoa` : ''}. ` +
